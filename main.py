...
class Main(MDApp):
    ...
    def change_theme(self):
        if self.theme_cls.theme_style =="Dark":
            ...
            if self.screen_manager.current=='MainPage':
                Clock.schedule_once(lambda x: self.show_categories(values=self.all_categories))
    ...
    def detailsPage(self,val,category):
        # print(val)
        # print(category)
        self.screen_manager.get_screen('DetailsPage').ids.details_title.text = category.upper()
        value = val
    ...
    def show_categories(self,values=''):
        ...
        self.screen_manager.get_screen('MainPage').ids.Main_page.clear_widgets()
        if values=='':
            self.all_categories= db.child('Users').child(self.user).child('Categories').get()
        else:
