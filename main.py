...
class CircleCard( FakeCircularElevationBehavior, MDCard):
    pass
...
class Main(MDApp):
        ...
        def forgot_password_(email,spinner):
            if email=='':
                ...
                try:
                    ...
                    boxLayout=MDBoxLayout(spacing=dp(0))
                    link_label=MDLabel(
    ...
    def login(self,email,password,spinner=None):
        ...
        def login_(email,password,spinner):
            ...
            try:
                ...
                self.screen_manager.get_screen('UserPage').ids.user_name.text=self.user
    ...
    def bottom_navigation(self,item):
        # print(item.children[0].icon)
        self.icon=item.children[0].icon
        for i in self.builder.ids.bottom_nav.children:
            if i == item:
                i.md_bg_color=self.theme_cls.primary_color
                i.children[0].text_color=(1,1,1,1)
            else:
    ...
    def change_screen(self, screen):
        ...
        elif screen=='UserPage':
            self.icon='account-outline'


        print(self.icon)
        for i in self.builder.ids.bottom_nav.children:
            if i.children[0].icon == self.icon:
                i.md_bg_color=self.theme_cls.primary_color
                i.children[0].text_color=(1,1,1,1)
            else:
                i.md_bg_color=self.theme_cls.bg_normal
